defmodule Poxa.WebHook.HandlerTest do
  ...
  describe "handle_info/2" do
    ...
    test "disconnected is not saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :disconnected, channels: ~w(channel_1 channel_2)}}, [])
      assert EventTable.all == []
    end

    test "subscribed is not saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :subscribed, channel: "my_channel"}}, [])
      assert EventTable.all == []
    end

    test "unsubscribed is not saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :unsubscribed, channel: "my_channel"}}, [])
      assert EventTable.all == []
    end

    test "channel_vacated is saved and delayed" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :channel_vacated, channel: "my_channel"}}, [])
      assert EventTable.all == [
    ...
    test "channel_occupied is saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :channel_occupied, channel: "my_channel"}}, [])
      assert EventTable.all == [
    ...
    test "client_event_message is saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :client_event_message, socket_id: "socket_id", channels: ~w(channel_1 channel_2), name: "my_event", data: "data"}}, [])
      expected = [
    ...
    test "client_event_message with user_id is saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :client_event_message, socket_id: "socket_id", channels: ~w(channel_1 channel_2), name: "my_event", data: "data", user_id: "123"}}, [])
      expected = [
    ...
    test "member_added is saved" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :member_added, channel: "channel", user_id: "user_id"}}, [])
      assert EventTable.all == [
    ...
    test "member_removed is saved and delayed" do
      assert {:noreply, []} == handle_info({:internal_event, %{event: :member_removed, channel: "channel", user_id: "user_id"}}, [])
      assert EventTable.all == [
