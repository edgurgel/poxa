defmodule Poxa.PresenceSubscriptionTest do
  ...
  test "subscribe to a presence channel" do
    ...
    expect(Poxa.registry, :register!, fn "presence-channel", {"id123", "info456"} -> :registered end)
    expect(Poxa.Event, :notify, fn :member_added, %{channel: "presence-channel", user_id: "id123"} -> :ok end)
    expect(PusherEvent, :presence_member_added, fn "presence-channel", "id123", "info456" -> :event_message end)
  ...
  test "unsubscribe to presence channel being subscribed" do
    ...
    expect(Poxa.Event, :notify, fn :member_removed, %{channel: "presence-channel", user_id: :userid} -> :ok end)
    expect(PusherEvent, :presence_member_removed, fn "presence-channel", :userid -> :event_message end)
    expect(Poxa.registry, :send!, fn :event_message, "presence-channel", ^pid -> :event_message end)
